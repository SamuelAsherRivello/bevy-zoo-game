...
![Bevy Zoo Game running app screenshot](documentation/images/Workflow01.png)
...
| Build desktop | `scripts/other/CompileApp.ps1 -Action build -PackageName bevy-zoo-game -TargetDir target/run-app-desktop -Features fast-dev -WgpuBackend dx12 -NoFastLinker` |
| Run web | `scripts/other/RunAppWeb.ps1` |
| Check web | `scripts/other/RunAppWeb.ps1 -CheckOnly` |
| Export web bundle | `scripts/other/RunAppWeb.ps1 -Release -NoOpen -ExportOnly` |
...
| Web target | `scripts/other/RunAppWeb.ps1` builds `wasm32-unknown-unknown` into `target/run-app-web`, runs `wasm-bindgen`, copies `bevy/crates/game/assets` into the web bundle, serves the generated page on localhost, and opens the browser unless `-NoOpen` or `-ExportOnly` is used. |
...
| Workflow | Behavior |
| -------- | -------- |
| `.github/workflows/export-web-build-to-github-pages.yml` | Builds the release web bundle with `scripts/other/RunAppWeb.ps1 -Release -NoOpen -ExportOnly`, validates `Bevy Zoo Game` and `bevy_zoo_game_bg.wasm`, stores versioned Pages releases, updates `latest`, and deploys GitHub Pages. |
| `.github/workflows/perform-release.yml` | Increments `VERSION.txt`, updates Bevy crate versions, commits the release version, creates a tag, pushes normally, and creates the GitHub release. |

## Structure

| Path | Purpose |
| ---- | ------- |
| `bevy/crates/game` | Main Bevy game crate and executable for zoo-specific runtime behavior. |
| `bevy/crates/game/src/runtime/components` | Zoo-specific ECS data attached to entities. |
| `bevy/crates/game/src/runtime/resources` | Zoo-specific ECS resources, model paths, and scene state. |
| `bevy/crates/game/src/runtime/systems` | Zoo-specific setup, model browser, scene reload, and DebugHUD composition. |
| `bevy/crates/game/src/runtime/plugins` | Game plugin composition and runtime tests. |
| `bevy/crates/game/assets` | Runtime game assets, including models, audio, and shaders. |
| `bevy/crates/game/assets/models` | Runtime GLB model packs used by the zoo scene and model browser. |
...
Keep gameplay changes small and spec-driven. Reusable window, camera, DebugHUD, inspector, and diagnostic input behavior belongs in `bevy/crates/shared`; zoo-specific scene, model, and gameplay behavior belongs in `bevy/crates/game`.
